...
export function vendorDataToPayload(
  ...
  return {
    ...
    // Legacy-only: round-tripped for backward-compatible payloads; not collected in onboarding.
...
export function mergePayloadIntoVendorData(
  ...
  return {
    ...
    onboardingFlowVersion:
      typeof p.onboardingFlowVersion === "number" && Number.isFinite(p.onboardingFlowVersion)
        ? Math.floor(p.onboardingFlowVersion)
        : 1,
