...
export function MasterDetailLayout({ list, detail, hasSelection }: MasterDetailLayoutProps) {
  return (
    <div className="flex h-full min-h-0 flex-col gap-6 lg:flex-row lg:gap-8">
      <div className={`flex min-w-0 flex-1 flex-col lg:max-w-md ${hasSelection ? "hidden lg:flex" : ""}`}>
        {list}
      </div>
      <div className={`flex min-h-0 min-w-0 flex-1 flex-col ${hasSelection ? "" : "hidden lg:flex"}`}>
        {detail}
      </div>
    </div>
  );
